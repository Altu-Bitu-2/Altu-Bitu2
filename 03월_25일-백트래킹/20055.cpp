...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        ...
        belt[start].power--; //내구도 하나 감소
        if (belt[start].power == 0) {
            //올리는 위치의 내구도가 0이면 0인 벨트 칸의 개수 +1
            zero_power++;
        }
    }
}
...
int main() {
    //입출력속도 상승
    ios_base::sync_with_stdio(false); //사용하는 버퍼의 수 제한
    cin.tie(NULL); // 입출력을 번갈아 할 때 cin과 cout의 묶음을 풀어주기

    int n, k; //컨베이어 벨트의 길이 2<=n<=100, 벨트 칸의 개수 1<=k<=2n

    //입력
    cin >> n >> k;
    belt.assign(n * 2, {0, false}); //컨베이터 벨트 초기화, 길이가 2n, 내구도 0, 로봇 존재 false
    for (int i = 0; i < n * 2; i++) {
        //총길이 2n 벨트의 내구도 입력
        cin >> belt[i].power;
    }
    ...
    //연산
    int start = 0;    //올리는 위치 : 0번
    int end = n - 1;  //내리는 위치 : n-1번
    int step = 0;     //단계 수
    while (true) {
        start = minusPosition(n, start); //올리는 위치 하나 감소
        end = minusPosition(n, end); //내리는 위치 하나 감소
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false;
        }
        second(n, start, end); //내리는 위치부터 올리는 위치까지 검사
        third(start); //올리는 위치가 내구도 0이 아니라면 로봇 존재 여부 체크 + 내구도 감소
        step++; // 단계 +1
        if (zero_power >= k) { // 만약 내구도가 0인 벨트칸이 k이상이면 종료
            break;
        }
    }
