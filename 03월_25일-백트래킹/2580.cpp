...
//수도쿠가 모두 채워졌는지 리턴하는 함수
bool fillSudoku(int idx) {
    if (idx == MAX * MAX) { //스도쿠 모두 채움 (기저 조건)
        return true; 
    }
    int row = idx / MAX; //현재 행
    int col = idx % MAX; //현재 열
    ...
    for (int i = 1; i <= MAX; i++) { //1~9까지 넣어보기
        if (!check_row[row][i] && !check_col[col][i] && !check_3x3[(row / 3) * 3 + col / 3][i]) {
            //만약 i행의 숫자가 존재하지 않고 i열의 숫자가 존재하지 않고 3x3 사각형 안에 숫자가 존재하지 않으면 
            check_row[row][i] = true; //행에 숫자 채움
            check_col[col][i] = true; //열에 숫자 채움
            check_3x3[(row / 3) * 3 + col / 3][i] = true; //사각형에도 체크
            sudoku[row][col] = i; //스도쿠 행렬에 숫자 i넣기
            if (fillSudoku(idx + 1)) {
                //현재 인덱스+1에 스도쿠가 모두 채워졌다면 true리턴
                return true; //생각해보기 : 이 부분이 없으면 어떻게 될까요?
            }
            check_row[row][i] = false; //다시 원래대로 돌려놓기
            check_col[col][i] = false; //false로 초기화
            check_3x3[(row / 3) * 3 + col / 3][i] = false; //false로 초기화
            sudoku[row][col] = 0; //0으로 초기화
        }
    }
    //모두 채워지지 않았음
    return false;
...
int main() {
    ...
    for (int i = 0; i < MAX; i++) {
        for (int j = 0; j < MAX; j++) {
            cin >> sudoku[i][j]; //9*9 스도쿠 판에 숫자 입력
            if (sudoku[i][j]) { //스도쿠 상태 보고, check배열들 초기화
                check_row[i][sudoku[i][j]] = true; //행 초기화
                check_col[j][sudoku[i][j]] = true; //열 초기화
                check_3x3[(i / 3) * 3 + j / 3][sudoku[i][j]] = true; //사각형 초기화
            }
        }
    }
