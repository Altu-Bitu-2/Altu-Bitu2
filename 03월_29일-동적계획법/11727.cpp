...
using namespace std;
const int MOD = 10007; // 직사각형을 채우는 방법의 수를 나눌 수

// 2xn 직사각형을 1x2, 2x1, 2x2 타일로 채우는 방법의 수를 구하는 함수
int fillTile(int n) {
    vector<int> dp(n + 1, 0); //동적 계획법을 위한 벡터
    ...
    dp[1] = 1; // 2x1 하나
    dp[2] = 3; // 2x1 1개와 2x2한개 혹은 2x1 한개와 1x2한개 혹은 1x2 세개로 세 가지
    for (int i = 3; i <= n; i++) {
        dp[i] = dp[i - 1] + 2 * dp[i - 2]; //각각 너비 1, 2만큼을 뺀 타일에서 1, 2 너비 타일을 각각 더하는 경우
        dp[i] %= MOD; //구하는 과정에서 int 범위 초과할 수 있으므로 마지막에 한 번이 아니라 중간 중간 모듈러 연산을 해줘야 함
    }
    return dp[n]; //방법의 수를 리턴
}
