...
using namespace std;
typedef vector<vector<int>> matrix; //2차원 백터를 matrix로 초기화
typedef tuple<int, int, int> tp; //3가지 정보를 입력할 튜플

queue<tp> spring(matrix &land, deque<tp> &tree, queue<pair<int, int>> &breeding_tree) {
    queue<tp> dead_tree; //죽은 나무의 정보
    int size = tree.size(); //나무의 개수
    while (size--) { //모든 나무 검사
        int age = get<0>(tree.front()); //나이
        int row = get<1>(tree.front()); //행
        int col = get<2>(tree.front()); //열
        tree.pop_front(); //나무 제거

        if (land[row][col] < age) { //자신의 나이만큼 양분을 먹을 수 없다면
            dead_tree.push({age, row, col}); //죽은 나무로 추가
            continue;
        }
        land[row][col] -= age; //나무가 자신의 나이만큼 양분을 먹고
        tree.emplace_back(age + 1, row, col); //나이가 1 증가함
        if ((age + 1) % 5 == 0) { //나이가 5의 배수라면
            breeding_tree.push({row, col}); //나무가 번식함
        }
    }
    return dead_tree; //죽은 나무의 수 리턴
}
...
void summer(queue<tp> &dead_tree, matrix &land) {
    while (!dead_tree.empty()) {
        ...
        dead_tree.pop(); //제거
        land[row][col] += (age / 2); //해당 땅의 양분을 죽은 나무의 나이/2만큼 추가
    }
}

void fall(int n, deque<tp> &tree, queue<pair<int, int>> &breeding_tree) {
    //인접한 8칸 : 상 하 좌 우 좌상향 좌하향 우상향 우하향
    ...
    while (!breeding_tree.empty()) {
        ...
        breeding_tree.pop(); //제거

        for (int j = 0; j < 8; j++) {
            //인접한 8칸 방문
            int nr = row + dr[j]; //다음 행
            int nc = col + dc[j]; //다음 열
...
int main() {
    //땅의 크기 n, 나무의 개수 m, 햇수 k, 심은 나무의 정보 위치(x, y), 나무의 나이 z
    int n, m, k, x, y, z;

    //입력
    cin >> n >> m >> k;
    matrix a(n, vector<int>(n, 0)); //로봇이 겨울에 주는 양분의 양
    matrix land(n, vector<int>(n, 5)); //땅의 처음 양분 모든 칸에 5
    queue<pair<int, int>> breeding_tree; //번식할 트리
    deque<tp> tree; //심은 나무의 정보
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            //겨울에 주는 땅의 양분 초기화
            cin >> a[i][j];
    ...
    //연산
    sort(tree.begin(), tree.end()); //어린 나이 순으로 정렬(오름차순)
    while (k--) {
        //햇수만큼 반복
        queue<tp> dead_tree = spring(land, tree, breeding_tree); //봄이 지나고 죽은 나무
        summer(dead_tree, land); //여름에 봄에 죽은 나무가 양분으로 변함
        fall(n, tree, breeding_tree); //가을엔 나무 번식
        winter(n, a, land); //겨울에는 땅에 양분을 추가
    }
