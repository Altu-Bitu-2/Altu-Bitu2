#include <iostream>
#include <vector>

using namespace std;
const int SIZE = 19; //열, 행 최댓값

//범위와 돌의 종류가 유효한지 확인
bool promising(int r, int c, int stone, vector<vector<int>> &board) {
    //공간 내부에 있고 돌이 위치했음
    return r >= 0 && r < SIZE && c >= 0 && c < SIZE && board[r][c] == stone;
}

bool validDir(int r, int c, int d, int stone, vector<vector<int>> &board) {
    //가로, 세로, 우하향 대각선, 우상향 대각선
    int dr[4] = {0, 1, 1, -1};
    int dc[4] = {1, 0, 1, 1};

    //(r, c) 이전에 위치한 이어지는 돌이 있나?
    if (promising(r - dr[d], c - dc[d], stone, board)) {
        //이어지는 돌이 없음
        return false;
    }
    int cnt = 0; //이어지는 돌 개수 카운트
    while (cnt < 6 && promising(r, c, stone, board)) { //(r, c)를 가장 왼쪽으로 하는 이어지는 바둑알의 개수
        cnt++; //이어지는 돌 +1
        r += dr[d]; //다음 row
        c += dc[d]; //다음 col
    }
    return cnt == 5; //이어지는 돌이 5개면 이김
}

bool isEnd(int r, int c, vector<vector<int>> &board) {
    for (int i = 0; i < 4; i++) { //가로, 세로, 우하향 대각선, 우상향 대각선 체크
        if (validDir(r, c, i, board[r][c], board)) {
            //이긴 상황이면 true
            return true;
        }
    }
    return false; //끝나지 않음
}

/**
 * [오목]
 *
 * 1. 특정 좌표(r, c)를 가장 왼쪽으로 하는 가능한 모든 오목 배치에 대해 오목 여부 확인
 *    가능한 모든 배치 :  오른쪽, 아래, 우하향, 우상향
 * 2. (주의) 육목이상이 되는지 확인해야 함
 *
 */

int main() {
    vector<vector<int>> board(SIZE, vector<int>(SIZE, 0)); //19x19 공간 초기화

    //입력
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            //공간 상태 입력
            cin >> board[i][j];
        }
    }

    //연산 & 출력
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            if (!board[i][j]) { //돌이 없음(0) -> 다음으로
                continue;
            }
            if (isEnd(i, j, board)) { //누군가 이겼나? -> 이겼으면true 출력
                cout << board[i][j] << '\n' << i + 1 << ' ' << j + 1;
                //1(흑), 2(백) -> 가로줄 번호와 세로줄 번호 출력, 무승부(0)
                return 0;
            }
        }
    }
    cout << 0;
    return 0;
}