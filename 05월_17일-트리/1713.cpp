...
bool cmp(const pair<int, ci> &c1, const pair<int, ci> &c2) {
    ...
    if (c1.second.first != c2.second.first) {
        //게시 시간(int)가 같지 않으면
        return c1.second.first < c2.second.first; //후자가 더 크면 1, 아니면 0
    }
    return c1.second.second < c2.second.second; //게시 시간이 같아도 마찬가지로 출력
}
...
int main() {
    int n, m, input; //사진틀의 개수 n, 전체 학생의 총 추천 횟수 m, 추천받은 학생을 나타내는 번호 input
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (candidate.size() == n && candidate.find(input) == candidate.end()) {
            //후보 학생의 크기가 사진틀의 개수와 같고, 후보 학생을 찾아서
            candidate.erase(min_element(candidate.begin(), candidate.end(), cmp));
            //추천 받은 횟수가 가장 적은 학생을 삭제(내림차순 정렬 후)
        }
        //첫 게시라면
        if (candidate.find(input) == candidate.end()) {
            //후보 학생을 찾으면
            candidate[input].second = i; //해당 후보 학생의 게시 시간 삽입
        }
